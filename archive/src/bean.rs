...
pub trait Bean {
    ...
    /// Calls all of the initiation methods on a bean to reduce boilerplate.
    /// Override to change the functionality and remove unneeded calls on your per case basis to improve performance.
    ...
    /// Calls all of the update methods on a bean to reduce boilerplate.
    /// Override to change the functionality and remove unneeded calls on your per case basis to improve performance.
    ...
    /// Calls all of the draw methods on a bean to reduce boilerplate.
    /// Override to change functionality and remove unneeded calls on your per case basis to improve performance.
    ...
    /// If your bean depends on other child beans to operate, then feed them in through here.
    /// Otherwise, it's fine to return an empty vec
    fn return_dependencies(&mut self) -> Option<&mut Vec<Box<dyn Bean>>> {
        None
    }
    ...
    /// Runs once after the bean enters the scope,
    /// but unlike init(), it will only be called after all the children have run their init and ready functions
    #[allow(unused_variables)]
