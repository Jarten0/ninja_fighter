...
pub fn debug_cli(root: &mut GameRoot) {
    ...
    let mut action_to_function: HashMap<&str, fn(&mut GameRoot) -> ()> = HashMap::new();

    let commands: Vec<(&str, fn(&mut GameRoot))> =
        vec![("help", help), ("savescene", save), ("loadscene", load)];
...
fn help(_: &mut GameRoot) {
    println!("{}", HOME_HELP_TEXT)
}

fn save(root: &mut GameRoot) {
